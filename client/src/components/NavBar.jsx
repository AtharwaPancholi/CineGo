  ...
  return (
    ...
    px-6 md:px-16 lg:px-36 py-5'> 
     ...
     border-gray-300/20 overflow-hidden transition-[width] duration-300 ${isOpen ? 'max-md:w-full' : 'max-md:w-0'}`}>
        <XIcon className='md:hidden absolute
        top-6 right-6 w-6 h-6 cursor-pointer'
     ...
     <div className='flex items-center gap-8'>
        ...
        {
          !user ? (<button onClick={openSignIn} className='px-4 py-1
        sm:px-7 sm:py-2 bg-primary hover:bg-primary-dull
     ...
     md:hidden w-8 h-8 cursor-pointer'
     onClick={()=> setIsOpen(!isOpen)} />
    </div>
  )
}
